...
void ZigbeeGateway::zbAttributeRead(uint16_t cluster_id, const esp_zb_ieee_addr_t long_address, const esp_zb_zcl_attribute_t *attribute)
{

  log_i("cluster_id %d ", cluster_id);
  log_i("attribute->id %d ", attribute->id);
  log_i("attribute->data_type %d ", attribute->data.type);
  _on_new_value_receive(cluster_id, long_address, attribute);
}
